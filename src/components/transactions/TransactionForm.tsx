...
export function TransactionForm({ onClose, onSaved, initialType = "expense", editing }: TransactionFormProps) {
  ...
  async function handleAttachment(file: File) {
    ...
    try {
      ...
      const r = storageRef(storage, `households/${householdId}/receipts/${profile.uid}/${Date.now()}_${file.name.replace(/[^\w.\-]/g, "_")}`);
      await uploadBytes(r, file);
      const url = await getDownloadURL(r);
      setAttachmentUrl(url);
      toast.success("Receipt attached");
    } catch {
      toast.error("Couldn't upload the receipt. Is Firebase Storage enabled?");
    } finally {
      setUploading(false);
    }
  }
