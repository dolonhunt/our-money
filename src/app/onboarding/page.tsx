...
export default function OnboardingPage() {
  ...
  async function uploadAvatar(file: File) {
    ...
    try {
      ...
      const r = storageRef(storage, `users/${user!.uid}/avatar/${Date.now()}_${file.name.replace(/[^\w.\-]/g, "_")}`);
      await uploadBytes(r, file);
      const url = await getDownloadURL(r);
      setPhotoURL(url);
    } catch {
      toast.error("Couldn't upload the image. Is Firebase Storage enabled?");
    } finally {
      setBusy(false);
    }
  }
  ...
  async function createSpace() {
    ...
    setError(null);
    try {
      const id = await createHousehold({ ...profile!, displayName: name.trim() || profile!.displayName, photoURL }, householdName.trim(), currency);
      const { getDoc, doc, getFirestore } = await import("firebase/firestore");
      const snap = await getDoc(doc(getFirestore(getFirebaseApp()), "households", id));
      const inviteCode = (snap.data() as { inviteCode: string })?.inviteCode ?? "";
